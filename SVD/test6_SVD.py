...
bscan_flat2[:,:] = bscan_flat[:,:]
#bscan_flat3[:,:] = bscan_flat2[:,:]
...
k = np.arange(0,41*kp,kp)

print(k.shape)

for m in np.arange(0,len(k),1):
    for i in np.arange(0,4596,1):
#   print("i=",i)
        bscan_flat2[i,m] = bscan_flat[i+k[m],m]
n = 4.5

plt.subplot(1,2,1)
plt.imshow(bscan_flat
           , aspect = 'auto' 
           , interpolation = 'none'           
)
plt.clim(-10**(n),10**(n))
#plt.axhline(y=1918,color='red', ls = "dotted")
plt.axhline(y=2030,color='red', ls = "dotted")



plt.subplot(1,2,2)
plt.imshow(bscan_flat2 
           , aspect = 'auto' 
           , interpolation = 'none'
